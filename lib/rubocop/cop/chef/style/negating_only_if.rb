...
module RuboCop
  module Cop
    module Chef
      module ChefStyle
        # Use not_if instead of only_if with a !
        #
        # @example
        #
        #   # bad
        ...
        class NegatingOnlyIf < Cop
          MSG = 'Use not_if instead of only_if with a !'.freeze
          ...
          def autocorrect(node)
            negated_only_if?(node) do |only_if, code|
              lambda do |corrector|
                corrector.replace(only_if.loc.expression, 'not_if')

                corrector.replace(code.parent.loc.expression, code.source.gsub(/^!/, ''))
              end
            end
          end
        end
